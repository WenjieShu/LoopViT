...
class LoopARCViT(nn.Module):
    ...
    def __init__(
        ...
        # Since we only support Standard ViT in this release:
        if HookLayerV0 is not None:
            LayerClass = HookLayerV0
            print(f"[LoopARCViT_Vis] Swapping to {LayerClass.__name__} for visualization.")
        else:
            LayerClass = target_cls
            print(f"[LoopARCViT_Vis] Using provided layer {LayerClass.__name__} (No Hook available).")
