...
class LoopForwardMetadata:
    """Container for loop-specific outputs returned by :class:`LoopARCViT`."""
...
class LoopARCViT(nn.Module):
    ...
    Args:
        num_tasks: Number of ARC tasks (determines size of task-token embedding).
        image_size: Canvas size (inputs are padded to ``image_size x image_size``).
        num_colors: Size of discrete color vocabulary.
        embed_dim: Hidden size of the transformer.
        loop_core_depth: Number of unique transformer layers in the shared trunk.
        max_loop_steps: Maximum number of times to reuse the shared trunk.
        min_loop_steps: Minimum number of steps before the exit gate may stop.
        num_heads: Attention heads per layer.
        mlp_dim: Hidden dimension of the FFN.
        dropout: Dropout probability used across the module.
        num_task_tokens: Number of learned task tokens prepended to the sequence.
        patch_size: ViT patch size. ``None`` falls back to pixel-level patches.
        use_exit_gate: Whether to predict a per-step exit probability.
        gate_threshold: Default gating threshold used when ``dynamic_exit=True``.
        add_step_embeddings: If True, add a learned embedding indicating the
            current loop iteration to the hidden states.
    """
